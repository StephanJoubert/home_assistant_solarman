            ...
            for request in requests:
                ...
                while attempts_left > 0:
                    ...
                    try:
                        ...
                        result = 0
                        log.warning(
                            f"Querying [{range_string}] failed with exception [{type(e).__name__}: {e}]"
                        )
                        self.disconnect_from_server()
                    if result == 0:
                        log.warning(
                            f"Querying [{range_string}] failed, [{attempts_left}] retry attempts left"
                        )
                    else:
                ...
                if result == 0:
                    log.warning(
                        f"Querying registers [{range_string}] failed, aborting."
                    )
                    break
